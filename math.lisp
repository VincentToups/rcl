...
;;#-linux (cffi:defcfun "Rf_gamma_cody" :double (a :double))
...
;;  exp(-x)_I(x;_nu) or exp(x)_K(x;_nu) if expo is 2. 
...
;; FIXME: what's wrong with this function? (at least with R 2.5.0)
;; undefined alien error in SBCL 
;; no dynamic object named expm1 warning in CLISP
#-cffi-features:windows (cffi:defcfun "expm1" :double (x :double))
...
;;#-linux (cffi:defcfun "Rf_pbeta_raw" :double (x :double) (a :double) (b :double) (lower-tail :int))
...
;;#-linux (cffi:defcfun "Rf_qchisq_appr" :double (p :double) (df :double) (lgamma :double) (lower-tail :int) (log-p :int) (tol :double))
